...
class TestConvolutionModel(unittest.TestCase):    
    ...
    def setUp(self):
        ...
        self.conv_model_obj  =  km.ConvolutionModel(
                                         x = self.x,
                                         y = self.y,
                                         vocab_size = 8000,
                                         num_timesteps = 8,
                                         ...
                                         embed_dim = 200,
                                         model_type = 'CNN-rand')
        self.conv_model_obj.compile_model()                      #for testing save methods
        self.conv_model_architecture = self.conv_model_obj.build_model_architecture()

    def test_conv_model_object_returns_constructor_params(self):
        self.assertTrue(np.array_equal(self.conv_model_obj.x,self.x))
        self.assertTrue(np.array_equal(self.conv_model_obj.y,self.y))
        self.assertEqual(self.conv_model_obj.vocab_size,8000)
        self.assertEqual(self.conv_model_obj.num_timesteps,8)
        self.assertEqual(self.conv_model_obj.batch_size,128)
        self.assertEqual(self.conv_model_obj.num_epochs,1)
        self.assertEqual(self.conv_model_obj.validation_split,.4)
    ...
    def test_get_model(self):
        conv_test_model = self.conv_model_obj.get_model()
        self.assertIsInstance(conv_test_model,Model)

    def test_copy_training_data(self):
        x_copy,y_copy = self.conv_model_obj.copy_training_data()
    ...
    def test_shuffle_data(self):
        data_shuffled = self.conv_model_obj.shuffle_data(self.x)
    ...
    def test_fit_model(self):
        self.assertIsInstance(self.conv_model_obj.fit_model(),keras.callbacks.History)

    def test_create_data_validation_split(self):
        x_train,y_train,x_val,y_val = self.conv_model_obj.create_data_validation_split()
