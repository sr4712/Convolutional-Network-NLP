...
class TestKerasTextListPreparer(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.text_list= [self.string1,self.string2]
            self.sentence_label = [0,1]
            self.keras_text_list_preparer_obj = textp.KerasTextListPreparer(
                    self.text_list,'SENTENCE_START','SENTENCE_END','UNKNOWN_TOKEN','PAD_TOKEN',10,12)
        ...
        def test_create_word_to_index_dict(self):
            ...
            self.assertCountEqual(
                            word_to_index_dict,
                            {'PAD_TOKEN': 0, 'an': 1, 'SENTENCE_START': 2, 'stopped': 7, 
        ...
        def test_create_index_to_word_dict(self):
            ...
            self.assertCountEqual(
                             index_to_word_dict,
                             {0: 'PAD_TOKEN', 1: 'an', 2: 'SENTENCE_START',
        ...
        def test_check_word_to_index_value(self):
            ...
            self.assertEqual(word_to_index_dict_test[self.keras_text_list_preparer_obj.pad_token],0)

        def test_create_training_data(self):
            keras_text_list_preparer_obj = textp.KerasTextListPreparer(
                    self.text_list,'SENTENCE_START','SENTENCE_END','UNKNOWN_TOKEN','PAD_TOKEN',10,12,self.sentence_label)
        ...
        def test_create_padded_training_data(self):            
            keras_text_list_preparer_obj = textp.KerasTextListPreparer(
                    self.text_list,'SENTENCE_START','SENTENCE_END','UNKNOWN_TOKEN','PAD_TOKEN',10,14)
            keras_text_list_preparer_obj.word_to_index_dict = {
                              'an': 1, 'SENTENCE_START': 2, 'stopped': 7, 
                              'the': 3, 'assessment': 9, 'SENTENCE_END': 4, 
                              'UNKNOWN_TOKEN': 10, 'governor': 8, 'could': 5,
            ...
            print(x_pad_training_data[0]) 
            print(x_pad_training_data[1])                                  
            self.assertCountEqual(
                                  x_pad_training_data[0],
                                  [2, 3, 10, 5, 6, 10, 1, 9, 10, 10, 10, 4, 0, 0])
            self.assertCountEqual(
